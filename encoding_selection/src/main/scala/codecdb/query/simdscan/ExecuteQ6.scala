...
object ExecuteQ6 extends App {
  ...
  val time = ParquetReaderHelper.profile(new File(args(0)).toURI, new EncReaderProcessor {
    override def processRowGroup(version: VersionParser.ParsedVersion,
      ...
      val quantityBitmap = new RoaringBitmap
      for (i <- 0L until quantityReader.getTotalValueCount) {
        val quan = quantityReader.getInteger
        if (quan < 24) {
          quantityBitmap.set(i, true)
        }
      ...
      val shipDateReader = new ColumnReaderImpl(shipDateCol, rowGroup.getPageReader(shipDateCol),
        new NonePrimitiveConverter, version)
      val shipDateBitmap = new RoaringBitmap
      for (i <- 0L until shipDateReader.getTotalValueCount) {
        val date = shipDateReader.getBinary.toStringUsingUTF8
        if (date.compareTo("1998-09-01") <= 0 && date.compareTo("1997-09-01") >= 0) {
          shipDateBitmap.set(i, true)
        }
      ...
      val bitmap = discountBitmap.and(shipDateBitmap).and(quantityBitmap)
      profiler.pause
